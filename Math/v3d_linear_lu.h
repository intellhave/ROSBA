   ...
   */
   ...
   class LU
   {
      ...
      private:
         ...
         void permute_copy( const Matrix<Real> &A, const Vector<int> &piv, int j0, int j1, Matrix<Real> &X)
         {
            int piv_length = piv.size();

            X.newsize(piv_length, j1-j0+1);
      ...
      public :
         ...
         */
         ...
         void solve (const Matrix<Real> &B, Matrix<Real> &X)
         {

            /* Dimensions: A is mxn, X is nxk, B is mxk */
      
            if (B.num_rows() != m) return;
            if (!isNonsingular()) return;
   ...
   invertMatrix(Matrix<double> const& A, Matrix<double>& Ainv)
   {
      ...
      Matrix<double> I(N, N, 0.0); for (int i = 0; i < N; ++i) I[i][i] = 1.0;
      LU<double> lu(A);
      Ainv.newsize(N, N);
      lu.solve(I, Ainv);
   }
