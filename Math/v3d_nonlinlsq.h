...
#define NLSQ_MAX_PARAM_TYPES 32
   ...
   struct NLSQ_Residuals
   {
         NLSQ_Residuals(vector<int> const& usedParamTypes, int const nMeasurements,
            ...
            : _residuals(nMeasurements, measurementDimension),
              ...
              _prevJs(usedParamTypes.size()),
              _errors(nMeasurements),
              _sto_residuals(nMeasurements, measurementDimension),
              _prev_residuals(nMeasurements, measurementDimension),
              _temp_residuals(nMeasurements, measurementDimension)
         {
            for (int k = 0; k < usedParamTypes.size(); ++k)
            {
               int const paramType = usedParamTypes[k];
               ...
               _prevJs[k] = new MatrixArray<double>(nMeasurements, measurementDimension, paramDimension);
            } // end for (k)
         } // end NLSQ_Residuals()
         ...
         ~NLSQ_Residuals()
         {
            ...
            for (int i = 0; i < _prevJs.size(); ++i) delete _prevJs[i];
         }

         VectorArray<double> _residuals;
         Vector<double>      _weights;
         
         //Added for stochastic operations
         Vector<double> _errors;
         VectorArray<double> _sto_residuals;
         VectorArray<double> _prev_residuals;
         VectorArray<double> _temp_residuals;


         vector<MatrixArray<double> * > _Js;
         //This is for MISO
         vector<MatrixArray<double> * > _prevJs;
   ...
   struct NLSQ_CostFunction
   {
         ...
         virtual void  preStoIterationCallback(std::vector<int>const &subset_idx) {}
         virtual void  initializeStoResiduals(std::vector<int>const &subset_idx) {}
         virtual void  evalStoResidual(int const k, Vector<double>& residual) const {}
         ...
         virtual void fillStoJacobian(int const whichParam, int const paramIx, int const k, Matrix<double>& J) const {}
         ...
         virtual void fillSubsetJacobians(Vector<double> const& weights, vector<MatrixArray<double> *>&Js, vector<int>const &subset_idx) const {}
         ...
         virtual int getNumParamBlocks(int const whichParam) {return -1;}
