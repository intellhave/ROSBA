...
# endif
   ...
   inline double psi(double const tau, double const r2)                        { return r2; }
   inline double psi_deriv(double const tau, double const r2)                  { return 1.0; }
   inline double psi_deriv2(double const tau, double const r)                  { return 0; }
...
//**********************************************************************
   ...
   struct BundleCost_Base
   {
         BundleCost_Base(int const mode, double const inlierThreshold,
            ...
            : _mode(mode), _cams(cams), _distortions(distortions), _Xs(Xs), _measurements(measurements), _correspondingParams_base(correspondingParams),
              ...
              _dp_dk1k2(measurements.size()), _dp_df(measurements.size()), _dp_dRT(measurements.size()), _dp_dX(measurements.size()), _residuals(measurements.size()),
                _sto_dp_dk1k2(measurements.size()), _sto_dp_df(measurements.size()), _sto_dp_dRT(measurements.size()), _sto_dp_dX(measurements.size()), _sto_residuals(measurements.size())
      ...
      int getNumCams(){return _cams.size();}
      int getNumPoitns() {return _Xs.size();}
      int getNumMeasurements() { return _measurements.size();}
      ...
      protected:
         ...
         void precompute_subset_residuals(std::vector<int>const &subset_idx)
         {
             int const K = subset_idx.size();
             for (int j = 0; j < K; ++j)
             {
                 int k = subset_idx[j];
                 int const view  = _correspondingParams_base[k][0];
...
#if !defined(USE_LINEARIZED_BUNDLE)
                 Vector2d const q = this->projectPoint(_Xs[point], view);
                 _sto_residuals[k] = q - _measurements[k];
#else
                 Vector3d const delta_X = _Xs[point] - _Xs0[point];
                 Vector3d const delta_T = _cams[view].getTranslation() - _cams0[view].getTranslation();
                 InlineVector<double, 6> delta_RT; makeZeroVector(delta_RT);
         ...
         void precompute_subset_bundle_derivatives(std::vector<int>const &subset_idx, bool precompute = precompute_default_arg)
         {
             if (!precompute) return;
             ...
             for (int j = 0; j < K; ++j)
             {
                 ...
                 switch (_mode)
                 {
                     case FULL_BUNDLE_RADIAL:
                         {
                             Matrix2x2d dxd_dk1k2 = _distortions[view].derivativeWrtRadialParameters(xu);
                             _sto_dp_dk1k2[k] = dp_dxd * dxd_dk1k2;
                             // No break here!
                         }
                     case FULL_BUNDLE_FOCAL_LENGTH:
                         {
                     ...
                     case FULL_BUNDLE_NO_ROTATIONS:
                         {
                             Matrix2x6d& dp_dRT = _sto_dp_dRT[k];
                             multiply_A_B(dp_dXX, dXX_dRT, dp_dRT);
                         }
                 } // end switch

                 // Jacobian w.r.t. 3d points
                 multiply_A_B(dp_dXX, dXX_dX, _sto_dp_dX[k]);
             } // end for (k)
         } // end precompute_bundle_derivatives()
        ...
        void copy_sto_camera_Jacobian(int const k, Matrix<double>& Jdst) const
         {
            switch (_mode)
            {
               case FULL_BUNDLE_RADIAL:
               {
                  copyMatrixSlice(_sto_dp_dk1k2[k], 0, 0, 2, 2, Jdst, 0, 7);
                  // No break here!
               }
               case FULL_BUNDLE_FOCAL_LENGTH:
               {
                  Jdst[0][6] = _sto_dp_df[k][0];
                  Jdst[1][6] = _sto_dp_df[k][1];
                  // No break here!
               }
               case FULL_BUNDLE_METRIC:
               {
                  copyMatrixSlice(_sto_dp_dRT[k], 0, 0, 2, 6, Jdst, 0, 0);
                  break;
               }
               case FULL_BUNDLE_NO_ROTATIONS:
               {
                  copyMatrixSlice(_sto_dp_dRT[k], 0, 0, 2, 3, Jdst, 0, 0);
               }
            } // end switch
         } // end copy_camera_Jacobian()
         ...
         //For stochastic -- not to mess with original  
         vector<Matrix2x2d> _sto_dp_dk1k2;
         vector<Vector2d>   _sto_dp_df;
         vector<Matrix2x6d> _sto_dp_dRT;
         vector<Matrix2x3d> _sto_dp_dX;
         vector<Matrix2x2d> _sto_dpsi_dp;
         vector<Vector2d>   _sto_residuals;
#if defined(USE_LINEARIZED_BUNDLE)
